...
The `meals` table has `id`, `caption`, `photo_path`, `items`, `total_calories`, `total_protein`, `usage_cost`, `confidence`, and `created_at`. Store `items` as JSON with `name`, `portion`, `calories`, and whole-gram `protein` on every item. Store the item sum in `total_protein`, leave `usage_cost` to the finish hook, store `created_at` as Unix milliseconds, and use an upsert by `id` for corrections or repairs.
