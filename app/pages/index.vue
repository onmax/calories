...
const { data } = await useFetch<{ meals: Meal[] }>("/api/meals");
...
const monthDailyAverage = computed(() =>
  loggedMonthDays.value ? Math.round(monthCalories.value / loggedMonthDays.value) : 0,
);
const selectedDayMeals = computed(() =>
  meals.value.filter((meal) => dateKey(meal.createdAt) === selectedDayKey.value),
);
const selectedDayCalories = computed(() =>
  selectedDayMeals.value.reduce((sum, meal) => sum + (meal.totalCalories ?? 0), 0),
);
const selectedMeal = computed(() =>
  selectedDayMeals.value.find((meal) => meal.id === selectedId.value),
);
...
<template>
  <main class="calories-app">
    <AppHeader :average="monthDailyAverage" :month-label="monthLabel" :total="monthCalories" />
