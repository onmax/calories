...
test("prior-entry actions identify one database record before acting", () => {
  ...
  assert.match(instructions, /convert `created_at` from Unix milliseconds/i)
  assert.match(instructions, /never infer a total from conversation history/i)
})
...
test("the Agent Definition delegates storage, cost, delivery, and rendering to ViteHub", () => {
  ...
  assert.match(agent, /callSettings:\s*\{ maxRetries: 2 \}/)
